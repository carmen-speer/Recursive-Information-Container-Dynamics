...
def render_row(d: dict) -> str:
    ...
    # confirmed outcomes (see the panel table above). This is a live
    # classifier *flag*, not a confirmed real-world outcome, so it gets
    # its own class -- "flagged" -- sharing the same red styling without
    # asserting the disputed thing in the markup itself. See the
    # Documentation standard section of the README.
    pred_class = "flagged" if prediction == "high_risk" else ("stable" if prediction == "stable" else "")
    probability = d.get("probability")
    prob_str = f"{probability * 100:.1f}%" if isinstance(probability, (int, float)) else "—"
    method = d.get("method") or "—"
    anchor_status = d.get("anchor_status") or "—"
